...
static void SendTxData(void)
{
  ...
  if (LmHandlerIsBusy() == false)
  {
	    ...
	    union {
	     ...
	     // Dichiarazione dell'union per il float della pressione
	     union {
	       float f;
	       uint8_t bytes[4];
	     } pressUnion;

	     // Assegna i valori ai campi float
	     tempUnion.f = tempC;
	     pressUnion.f = press;

	     // Copia i 4 byte di tempC nel buffer
	     for (uint8_t j = 0; j < 4; j++) {
	       AppData.Buffer[i++] = tempUnion.bytes[j];
	     }
	     // Copia i 4 byte di press nel buffer
	     for (uint8_t j = 0; j < 4; j++) {
	      AppData.Buffer[i++] = pressUnion.bytes[j];
	     }
